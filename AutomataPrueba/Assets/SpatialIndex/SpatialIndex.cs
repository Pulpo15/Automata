        ...
        if (Application.isEditor)
        {
            ...
            for (int i = 0; i < numCubesX; i++)
            {
                for (int j = 0; j < numCubesZ; j++)
                {
                    ...
                    else if ((floor[i * numCubesX + j] & FLOOR_STATUS.ICE) == FLOOR_STATUS.ICE)
                    {
    ...
    void serializeMap()
    {
        ...
        for (int i = 0; i < numCubesX; i++)
        {
            for (int j = 0; j < numCubesZ; j++)
            {
                ...
                else if (Physics.CheckBox(center + transform.rotation * Vector3.right * cubeSize * i + transform.rotation * Vector3.forward * j, Vector3.one * cubeSize / 2.0f, transform.rotation, 1 << 12))
                {
                    floor[numCubesX * i + j] = FLOOR_STATUS.ICE;
                }
                else
